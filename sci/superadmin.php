...
<?php
$password = $su_pass;
$nonsense = "AwwrreEShayAnBAunjAhdiFxpialidocqwes";

if (isset($_COOKIE['PrivatePageLogin_sudo'])) {
   if ($_COOKIE['PrivatePageLogin_sudo'] == md5($password.$nonsense)) {
?>


<!-- PUT THE LOGGED IN CODE HERE-->
<?php include_once 'superadmin_auth.php'; ?>
...
if (isset($_GET['p']) && $_GET['p'] == "login") {
    if ($_POST['keypass'] != $password) {
      echo '<script> $(window).load(function () {sweetAlert({
			...
			function () {    window.location = "superadmin.php"; });})</script>';
            $line = date('Y-m-d H:i:s') . " - $_SERVER[REMOTE_ADDR]" . " - " . $_POST['keypass'];
            file_put_contents('wrongpass_su.log', $line . PHP_EOL, FILE_APPEND);
      exit;
   } elseif ($_POST['keypass'] == $password) {
      setcookie('PrivatePageLogin_sudo', md5($_POST['keypass'].$nonsense));
